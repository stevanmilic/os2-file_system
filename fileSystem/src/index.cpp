...
void IndexAlloc::writeIndex(BytesCnt current){
	...
	//write to second level indexes
		...
		index[firstLevel][secondLevelIndex++] = pw->write(index[secondLevel],ClusterSize,0,entry);	
		memset(index[secondLevel],0,ClusterSize);
		dataCluster = 0;
	}
	index[indexCluster][dataCluster++] = pw->write(buffer + current,currSize,currOffset,entry);
}
...
void IndexAlloc::writeLast(){
	...
	pw->write(index[firstLevel],ClusterSize,entry,0,pw->getStartCluster(entry));
}
