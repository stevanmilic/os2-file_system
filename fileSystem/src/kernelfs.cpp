...
char KernelFS::kdelete(char* fpath, bool wait) {
	EntryNum index = kexist(fpath);
	if (index > 63)
		return 0;//except: file doesn't exist

	FCB* fcb = ft.findKey(FCBid(index, PartWrapper::parseName(fpath), 'd'));

	//if delete file is called from write method, wait till file is closed and delete it
	//else file opened so it is an error
	if (fcb != 0 && wait)
		fcb->startMode('w');
	...
	pw->fopen(index);
	KernelFile::kdeleteFile(pw, index);
	pw->fclose(index);
	Directory& myDir = pw->rootDir();
